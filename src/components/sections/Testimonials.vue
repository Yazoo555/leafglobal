<template>
  <section v-scroll-reveal="'fade-in'" class="section-padding" :class="darkBg ? 'gradient-dark' : 'bg-background'">
      ...
      <div class="mt-16">
        ...
        <div v-scroll-reveal.stagger="'slide-up'" class="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div
            ...
            :key="index"
            class="card-base card-hover p-8 flex flex-col"
            :class="darkBg ? 'bg-white' : ''"
          >
            <!-- Stars -->
            ...
            <p class="text-text-light text-sm leading-relaxed flex-1 italic">
              "{{ item.text }}"
            </p>
            <div class="flex items-center gap-3 pt-5 mt-5 border-t" :class="darkBg ? 'border-gray-100' : 'border-gray-100'">
              <!-- Avatar -->
              <div
                class="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0"
                :class="item.avatarColor || 'gradient-primary'"
              >
                {{ item.name.charAt(0) }}
              </div>
              <div>
                <div class="text-sm font-semibold" :class="darkBg ? 'text-white' : 'text-dark'">
                  {{ item.name }}
                </div>
                <div class="text-xs text-text-light">{{ item.role }}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="md:hidden relative">
          <div class="overflow-hidden">
            ...
            >
              ...
              >
                <div class="card-base p-8" :class="darkBg ? 'bg-white' : ''">
                  <div v-if="item.rating" class="flex items-center gap-1 mb-4">
                  ...
                  <Icon name="quote" size="20" class="text-primary/20 mb-3" />
                  <p class="text-text-light text-sm leading-relaxed italic">"{{ item.text }}"</p>
                  <div class="flex items-center gap-3 pt-5 mt-5 border-t border-gray-100">
                    <div class="w-10 h-10 rounded-full gradient-primary flex items-center justify-center text-white text-sm font-bold shrink-0">
                      {{ item.name.charAt(0) }}
                    </div>
                    <div>
                      <div class="text-sm font-semibold text-dark">{{ item.name }}</div>
                      <div class="text-xs text-text-light">{{ item.role }}</div>
                    </div>
                  </div>
                </div>
          ...
          <button
            ...
            class="absolute left-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-dark hover:text-primary transition-colors duration-200"
            @click="currentSlide--"
          ...
          <button
            ...
            class="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white shadow-md flex items-center justify-center text-dark hover:text-primary transition-colors duration-200"
            @click="currentSlide++"
...
const props = defineProps({
  ...
  /** How many testimonials to show in desktop grid */
